  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", position: "relative" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 14 }}>
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>Statutory Safety Audits & Compliance Reports</h2>
          <p style={{ fontSize: 12.5, color: "var(--text-muted)", margin: "3px 0 0" }}>Compile DGMS returns, shift inspection logs, and environmental telemetry reports.</p>
        </div>
        <button
        ...
        >
          <Plus size={15} /> {t("btn.generate_report", "Generate Report")}
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 20 }}>
        {[
          { label: "Compliance Score – May", value: "88%",     trend: "↑ 6.4%", positive: true, color: "#16a34a" },
          { label: "Total Inspections – May", value: "126",    trend: "↑ 10.2%", positive: true, color: "#2d6a4f" },
          { label: "Open Violations – May",   value: "28",     trend: "↓ 7.1%", positive: false, color: "#ea580c" },
        ].map(s => (
          <div
            key={s.label}
            style={{
          ...
          >
            ...
            <div>
              <p style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 2px" }}>{s.label}</p>
              <p style={{ fontSize: 26, fontWeight: 900, color: "var(--text-primary)", margin: 0, lineHeight: 1 }}>{s.value}</p>
            </div>
            <span style={{ fontSize: 12, fontWeight: 800, color: s.positive ? "#16a34a" : "#dc2626", background: s.positive ? "#dcfce7" : "#fee2e2", padding: "5px 10px", borderRadius: 8 }}>
      ...
      {/* Chart */}
      <div style={{ background: "white", border: "1px solid var(--border)", borderRadius: 14, padding: 20, marginBottom: 20, boxShadow: "var(--shadow-xs)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
          <div>
            <h3 style={{ fontSize: 14, fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>Monthly Performance Trends</h3>
            <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "2px 0 0" }}>Inspections, Violations & Compliance Score – Last 7 months</p>
          </div>
          <div style={{ display: "flex", gap: 16 }}>
            {[
              ...
              <div key={l.label} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 12, color: "var(--text-secondary)", fontWeight: 600 }}>{l.label}</span>
              </div>
            ))}
          </div>
        </div>
        <ResponsiveContainer width="100%" height={210}>
          <BarChart data={monthlyData} barCategoryGap="30%" barGap={4} margin={{ top: 0, right: 0, left: -24, bottom: 0 }}>
            ...
            <Bar dataKey="inspections" fill="#2d6a4f" radius={[4, 4, 0, 0]} />
            <Bar dataKey="violations"  fill="#dc2626" radius={[4, 4, 0, 0]} />
            <Bar dataKey="compliance"  fill="#f4a261" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div style={{ background: "white", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden", boxShadow: "var(--shadow-xs)" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h3 style={{ fontSize: 14, fontWeight: 800, margin: 0, color: "var(--text-primary)" }}>Report Library ({reportsList.length})</h3>
          <span style={{ fontSize: 11.5, color: "var(--text-muted)", fontWeight: 600 }}>Attested under DGMS Regulation 153</span>
        </div>
        {reportsList.map((r, i) => {
          ...
          return (
            <div
              ...
              style={{
                padding: "14px 20px",
                borderBottom: i < reportsList.length - 1 ? "1px solid var(--surface-2)" : "none",
                display: "flex",
                alignItems: "center",
                gap: 14,
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>{r.title}</span>
                  {r.status === "Draft" && (
                    <span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10.5, fontWeight: 800, background: "#fff7ed", color: "#ea580c" }}>Draft</span>
                  )}
                </div>
