  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12.5, color: "var(--text-muted)", display: "flex", alignItems: "center", gap: 6, margin: "4px 0 0" }}>
        ...
        {[
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: 24, fontWeight: 900, color: c.color, lineHeight: 1.1, margin: 0 }}>{c.value}</p>
            </div>
          </div>
        ))}
      </div>
          ...
          {colliery.sections.map(sec => {
            ...
            return (
              <div key={sec.name} style={{
                background: "white",
                border: `1px solid ${sec.status === "Restricted" ? "#fca5a5" : "var(--border)"}`,
                borderRadius: 14,
                overflow: "hidden",
                boxShadow: sec.status === "Restricted" ? "0 0 0 2px #fee2e2" : "var(--shadow-xs)",
                transition: "all 0.2s ease"
              }}>
                {/* Section header */}
                <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>{sec.name}</p>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 5 }}>
                    <span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10.5, fontWeight: 800, background: ss.bg, color: ss.color }}>
                      {sec.status}
                    </span>
                ...
                {/* Compliance bar */}
                <div style={{ padding: "10px 16px", borderBottom: "1px solid var(--surface-1)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                    <span style={{ fontSize: 11.5, color: "var(--text-muted)", fontWeight: 600 }}>Statutory Compliance</span>
                    <span style={{ fontSize: 12, fontWeight: 800, color: complianceColor(sec.compliance) }}>{sec.compliance}%</span>
                  </div>
                  <div style={{ height: 6, background: "var(--surface-2)", borderRadius: 4, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${sec.compliance}%`, background: complianceBg(sec.compliance), borderRadius: 4, transition: "width 0.6s ease" }} />
                  </div>
                </div>
