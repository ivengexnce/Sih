  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "#111827" }}>Corporate Team</h2>
          <p style={{ fontSize: 13, color: "#6b7280", marginTop: 2 }}>Executive dashboard for personnel management across all sites.</p>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>
        {[
          ...
          { label: "Total Reports",   value: executives.reduce((s, e) => s + e.reports, 0), color: "#2563eb", bg: "#eff6ff" },
          { label: "Sites Covered",   value: 5,                                             color: "#7c3aed", bg: "#fdf4ff" },
          { label: "Currently Active",value: executives.filter(e => e.status === "Active").length, color: "#16a34a", bg: "#dcfce7" },
        ].map(c => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 16 }}>
        {executives.map(exec => {
          ...
          return (
            <div key={exec.name} style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 12, padding: "18px 20px", display: "flex", gap: 16 }}>
              {/* Avatar */}
              <div style={{ width: 52, height: 52, borderRadius: 13, background: "#1a3d28", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: "#111827" }}>{exec.name}</p>
                    <p style={{ fontSize: 12.5, color: "#6b7280", marginTop: 2 }}>{exec.title}</p>
                  </div>
                  <span style={{ padding: "3px 9px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: exec.status === "Active" ? "#dcfce7" : "#fff7ed", color: exec.status === "Active" ? "#16a34a" : "#ea580c", whiteSpace: "nowrap" }}>
                    {exec.status}
                  </span>
                </div>
                ...
                <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 5 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 9px", borderRadius: 6, fontSize: 11.5, fontWeight: 600, background: dc.bg, color: dc.color }}>
                      {dc.icon} {exec.dept}
                    </span>
                    <span style={{ fontSize: 11.5, color: "#9ca3af" }}>📍 {exec.location}</span>
                  </div>
                  <div style={{ display: "flex", gap: 16, marginTop: 4 }}>
                    <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11.5, color: "#6b7280" }}><Mail size={11} />{exec.email}</span>
                  </div>
                  <div style={{ display: "flex", gap: 16 }}>
                    <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11.5, color: "#6b7280" }}><Phone size={11} />{exec.phone}</span>
                  </div>
                </div>

                <div style={{ marginTop: 12, display: "flex", gap: 20, paddingTop: 12, borderTop: "1px solid #f3f4f6" }}>
                  <div>
                    <p style={{ fontSize: 10.5, color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>Direct Reports</p>
                    <p style={{ fontSize: 16, fontWeight: 700, color: "#2d6a4f" }}>{exec.reports}</p>
                  </div>
                  <div>
                    <p style={{ fontSize: 10.5, color: "#9ca3af", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>Mine Sites</p>
                    <p style={{ fontSize: 16, fontWeight: 700, color: "#2563eb" }}>{exec.mines}</p>
                  </div>
                </div>
              </div>
            </div>
